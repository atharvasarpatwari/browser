...
export class Rasterizer {
   ...
   * Each call is a fresh frame — reset canvas state (clip rect, transform,
   * alpha, save stack) first. `save`/`restore` in the command stream only
   * brackets opacity/transform groups, never an overflow:hidden `clip`, so
   * without this reset a clip from one call (e.g. this page's first paint)
   * would silently persist into every later rasterize() call on this same
   * instance — including the next call's own full-canvas background fill —
   * clipping content that has nothing to do with the element that set it.
   */
