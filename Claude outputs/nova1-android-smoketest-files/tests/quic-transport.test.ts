...
describe('QuicConnection — encode side (exact wire bytes)', () => {
  it('sends a well-formed Initial packet carrying the handshake literal in a CRYPTO frame', async () => {
    ...
    // Long header, type encoded in bits 4-5 via quic-wire.ts's
    // buildLongHeaderPacket: 0xC0 | (Initial(0x00) & 0x30) === 0xC0 (Initial's
    // value is 0x00, so this holds regardless of which bit range encodes the
    // type — see the file header for the bug this used to describe here).
