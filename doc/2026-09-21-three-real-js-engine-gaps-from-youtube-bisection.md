...
**Status:** Completed (3 root causes fixed; YouTube still doesn't fully run — see Notes). **All remaining gaps noted below were resolved the next day — see the 2026-09-22 update in Notes.**
...
- **Update (2026-09-22):** every gap listed below as "still-open" is now resolved — see `doc/2026-09-22-five-more-js-engine-gaps-from-youtube-bisection.md`. `Cannot find global object` was `window`/`self`/`globalThis` never mirroring real built-ins; the `Cannot read properties of undefined` gaps were the same window-mirroring fix being a one-time snapshot instead of a live link, plus a genuinely missing `performance.timing`; `Expected Colon, got String` (below) was a regex-vs-`/=`-operator dispatch-order bug in a 10.7 MB external bundle, found via the "proper statement-boundary-aware bisection tool" this note called for — turned out the fix didn't need statement boundaries at all, just a continuous single-pass token scan for anomalously-shaped tokens.
