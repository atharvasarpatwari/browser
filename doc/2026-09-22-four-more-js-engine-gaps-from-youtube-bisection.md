# Four More Real JS-Engine Gaps Found Continuing the YouTube Bisection

**Date:** 2026-09-22
**Session:** Asked to continue with the "Cannot find global object" bug — one of the still-open gaps left documented (but unresolved) by the prior session's YouTube-bisection work (`doc/2026-09-21-three-real-js-engine-gaps-from-youtube-bisection.md`). After fixing it, asked to keep going through the gap list; continuing the same real-bisection methodology surfaced two more real, distinct bugs the same session.
**Status:** Completed (4 root causes fixed; a full real-`youtube.com` sweep of all 34 inline scripts now shows 0 parse failures and 0 runtime errors)

---

## Summary

Root-caused "Cannot find global object" to a real, well-known JS idiom (the UMD "find the true global object" feature-detection pattern) legitimately throwing its own designed error, because Nova's `window` never mirrored global built-ins and `self`/`globalThis` didn't exist at all. Fixing that let parsing/execution progress further into YouTube's bundle, which surfaced three more real bugs in turn: a regex escaped-slash pattern/flags corruption, global `var`/`window` bindings being two independently-updated copies instead of one live store, and a missing legacy `performance.timing` object. Each was confirmed against the exact real-world triggering source before being fixed, and re-confirmed against live `youtube.com` and in real Electron afterward. The full diagnostic now shows **zero** failing scripts out of YouTube's 34 real inline scripts (parse or runtime).
...
- Every fix was confirmed against a live fetch of `youtube.com`'s real HTML before any code was written (following the same direct-fetch-and-bisect methodology as the prior session), and re-confirmed the same way afterward.
- Each fix let execution progress further into scripts that previously aborted earlier, which is precisely what surfaced the next bug — not a separately-reported gap, but a bug that only became reachable once the previous one was fixed. Same "each fix moves the failure point later" pattern the prior session documented, chained four times in a row this session: window-mirroring → regex escaped-slash → global var/window sync → missing `performance.timing`. After the fourth fix, a full sweep of all 34 real inline scripts shows **0 parse failures and 0 runtime errors** — the first time this diagnostic has come back completely clean.
- Adding the new e2e regression checks initially made the entire `language-features` case fail (all 32 marks, not just the 4 new ones at the time) in the real Electron run. Root cause was a test-authoring mistake, not a Nova bug: two of the new checks' explanatory *comments* contained the literal text `</script>`, and the HTML tokenizer scans for that literal byte sequence to end a `<script>` element regardless of JS string/comment context — truncating the whole harness script mid-file. Fixed by rewording the comments to avoid the literal substring. Confirmed via `page.on('console'/'pageerror')` wiring (temporarily added, then removed) that the real Electron `ScriptEngine` was reporting `Expected RBrace, got EOF` — a truncated-script signature — which pointed straight at the real cause once the local `runJS`-only reproduction (which doesn't go through HTML tokenization) failed to show any problem.
...
| `src/browser/js/lexer.ts` | `readRegex()` returns `regexParts: { pattern, flags }` on the token instead of only the merged display string |
...
| `tests/e2e/js-dom-api-sweep.spec.ts` | 6 new real-Electron regression checks (UMD global-object pattern, window/self/globalThis identity + mirrored built-ins, escaped-slash regex with backreference, more escaped-slash shapes, live var/window sync in both directions, `performance.timing` fields) |
...
Real YouTube diagnostic (before → after)                 → 0/34 scripts fail to parse, 0/34 fail at runtime (previously: "Cannot find global object" ×4, regex corruption, 3 "Cannot read properties of undefined" errors)
```
...
5. Re-ran the full real-YouTube diagnostic a final time: 0/34 scripts fail to parse or execute.
6. Added 6 permanent regression checks to `tests/e2e/js-dom-api-sweep.spec.ts`; diagnosed and fixed an unrelated test-authoring mistake along the way (literal `</script>` in a comment truncating the harness page) using real Electron console/pageerror capture.
7. Rebuilt (`npm run build:web`) and re-ran the full real-Electron e2e suite, the full vitest suite, and a full typecheck for regressions.
